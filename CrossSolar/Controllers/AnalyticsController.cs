...
namespace CrossSolar.Controllers
{
    ...
    public class AnalyticsController : Controller
    {
        ...
        public async Task<IActionResult> DayResults([FromRoute] string panelId)
        {
            ...
            var data = analytics.GroupBy(e => e.DateTime).ToList() ;
            var result = new List<OneDayElectricityModel>();

            foreach (var item in data)
            {
                OneDayElectricityModel dayItem= new OneDayElectricityModel();
                dayItem.Maximum = 0;
                dayItem.Minimum = 0;
                dayItem.DateTime = item.FirstOrDefault().DateTime;
                dayItem.Sum = 0;
                foreach (var nestedItem in item.ToList())
                {
                    if (nestedItem.KiloWatt > dayItem.Maximum)
                        dayItem.Maximum = nestedItem.KiloWatt;
                    if (nestedItem.KiloWatt < dayItem.Minimum)
                        dayItem.Minimum = nestedItem.KiloWatt;
                    dayItem.Sum += nestedItem.KiloWatt;

                }
                if (item.ToList().Count() > 0)
                    dayItem.Average = dayItem.Sum / item.ToList().Count();
                result.Add(dayItem);
            }
